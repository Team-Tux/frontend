...
export default function TableExample() {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (statusFilter && statusFilter !== "all") {
      res = res.filter((r) => {
        if (statusFilter === "done")
          return r.status === "done" || r.status === "closed";
    ...
    if (delegateFilter && delegateFilter !== "All") {
      res = res.filter(
        (r) => String(r.delegated_to) === String(delegateFilter),
      );
    }
    ...
    if (orderBy === "reported") {
      ...
      res.sort(
        (a, b) =>
          (prioOrder[String(b.priority).toLowerCase()] || 0) -
          (prioOrder[String(a.priority).toLowerCase()] || 0),
      );
      ...
      res.sort((a, b) => {
        ...
        const oa = Object.prototype.hasOwnProperty.call(statusOrder, sa)
          ? statusOrder[sa]
          : 99;
        const ob = Object.prototype.hasOwnProperty.call(statusOrder, sb)
          ? statusOrder[sb]
          : 99;
  ...
  const delegateTo = (id, delegateId) => {
    setRows((prev) =>
      prev.map((r) => (r.id === id ? { ...r, delegated_to: delegateId } : r)),
    );
  ...
  const createNewDelegate = async () => {
    ...
    try {
      ...
      setDelegateMap((prev) => ({
        ...prev,
        [newDelegate.id]: newDelegate.name,
      }));
  ...
  const fetchImagesForIncident = async (incidentId, lat, lon) => {
    ...
    try {
      ...
      console.log(
        "✅ Found images for incident:",
        incidentId,
        mockExistingImages.length,
      );
      setIncidentImages((prev) => ({
        ...prev,
        [incidentId]: mockExistingImages,
      }));
    } catch (error) {
  ...
  return (
    <div>
      <div
        className="controls d-flex flex-wrap gap-3 mb-3 p-3 rounded shadow-sm align-items-end justify-content-between"
        style={{
            ...
            <div className="btn-row">
              <CDropdown>
                <CDropdownToggle color="secondary" className="btn-sm">
                  {delegateFilter === "All"
                    ? "All"
                    : getDelegateName(delegateFilter)}
                </CDropdownToggle>
                <CDropdownMenu>
                  <CDropdownItem
                  ...
                  {allDelegates.map((d) => (
                    <CDropdownItem
                      key={d.id}
                      onClick={() => setDelegateFilter(d.id)}
                      active={delegateFilter === d.id}
                    >
                      {d.name}
                    </CDropdownItem>
                  ))}
      ...
      <div
        className="overflow-hidden shadow-sm rounded-top"
        style={{
              ...
              return (
                <React.Fragment key={r.id}>
                  ...
                  >
                    ...
                    <CTableDataCell
                      style={{ ...commonCellStyle, textDecoration: "none" }}
                    >
                    ...
                    <CTableDataCell
                      style={{ ...commonCellStyle, textDecoration: "none" }}
                    >
                      {getDelegateName(r.delegated_to)}
                    </CTableDataCell>
                    <CTableDataCell
                      style={{ ...commonCellStyle, textDecoration: "none" }}
                    >
